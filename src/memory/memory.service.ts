...
export class MemoryService {
  ...
  async saveMemory(text: string, orgId: string): Promise<string> {
    ...
    try {
      ...
      if (
        lowerText.includes('ตกลง') ||
        lowerText.includes('agree') ||
        lowerText.includes('decided') ||
        lowerText.includes('สรุปว่า')
      ) {
        type = 'agreement';
      } else if (
        lowerText.includes('รับผิดชอบ') ||
    ...
    try {
      ...
      if (memories.length === 0) {
        ...
        if (allMemories.length === 0)
          return '🧠 ยังไม่มีการบันทึกใดๆ\nใช้ /note เพื่อบันทึก';

        return (
          `🧠 บันทึกทั้งหมด (${allMemories.length}):\n` +
          allMemories
            .map(
              (m, i) =>
                `${i + 1}. ${m.text} (${m.createdAt.toLocaleDateString('th-TH')})`,
            )
            .join('\n')
        );
      }
      ...
      return (
        `🤝 ข้อตกลงที่บันทึกไว้ (${memories.length}):\n` +
        memories
          .map(
            (m, i) =>
              `${i + 1}. ${m.text} (${m.createdAt.toLocaleDateString('th-TH')})`,
    ...
    try {
      ...
      if (memories.length === 0) {
        ...
        if (broadSearch.length === 0)
          return `🔍 ไม่พบข้อมูลเกี่ยวกับ "${project}"`;
