...
export class SubscriptionService {
  ...
  async getOrCreateOrg(
    orgId: string,
    isGroup: boolean,
  ): Promise<{ id: string; plan: PlanType }> {
    try {
      let org;
      if (isGroup) {
      ...
      try {
        ...
        if (existing)
          return { id: existing.id, plan: existing.plan as PlanType };
      } catch (e) {}
        ...
        case 'summary_yesterday':
          if (!limits.canSummaryYesterday) {
            return {
              allowed: false,
              message:
                '📋 สรุปเมื่อวาน เป็นฟีเจอร์สำหรับแผน Pro ขึ้นไป\n🔥 Pro ฿300/เดือน → สรุปวันนี้+เมื่อวาน',
            };
          }
          break;
        case 'assign_task':
          if (!limits.canAssignTasks) {
            return {
              allowed: false,
              message:
                '👤 มอบหมายงาน เป็นฟีเจอร์สำหรับแผน Basic ขึ้นไป\n⭐ Basic ฿200/เดือน',
            };
          }
          break;
      }
  ...
  async getPlanStatus(orgId: string, isGroup: boolean): Promise<string> {
    try {
      ...
      const aiLimit =
        limits.aiChatsPerDay >= 999999 ? '∞' : limits.aiChatsPerDay;
      const tasksUsed = org?.tasksThisMonth || 0;
      const taskLimit =
        limits.tasksPerMonth >= 999999 ? '∞' : limits.tasksPerMonth;
